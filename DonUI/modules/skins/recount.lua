...
local function SkinFrame(frame)
	...
	frame.Title:SetFont(C.media.uffont, 10, "THINOUTLINE")
	...
	frame.CloseButton.t:SetFont( C.media.uffont, 10, "THINOUTLINE")
...
Recount.UpdateBarTextures = function(self)
	for k, v in pairs(Recount.MainWindow.Rows) do
		...
		v.LeftText:SetFont(C.media.uffont, 10, "THINOUTLINE")
		v.RightText:SetPoint("RIGHT", -4, 1)
		v.RightText:SetFont(C.media.uffont, 10, "THINOUTLINE")
	end
end
...
for i = 1, getn(MWbuttons) do
	...
	if button then
		...
		button:FontString("text", C.media.uffont, 10, "THINOUTLINE")
